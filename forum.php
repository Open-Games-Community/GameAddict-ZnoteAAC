...
if ($admin && !empty($_POST)) {
	...
	if ($admin_category_edit !== false) {
		...
		if ($category !== false) {
			?>
			<form action="" method="post" style="width:775px">
	...
	if ($getThread !== false) {
		...
		if ($threadData !== false) {
			...
			if ($access) {
				...
				<table class="znoteTable ThreadTable table table-striped" style="width:775px">
					<tr class="yellow">
				...
				if ($admin || $leader) {
					...
					// $yourChars
					?>
					<table class="adminTable table" style="width:775px">
						<tr>
							<td>
					...
					if ($threadData['closed'] == 0 && PlayerHaveAccess($yourChars, $threadData['player_name'])) {
						?>
						<table class="editThread" style="width:775px">
							<tr>
								<td>
									<form action="" method="post">
										<input type="hidden" name="edit_thread_id" value="<?php echo $threadData['id']; ?>">
...
<p style="font-size: 13px; padding-left: 10px; padding-top: 10px; height: 5px; width: 600px; border-top: 1px solid black;"><b>[b]Bold Text[/b]</b>, [img]<a href="https://imgur.com/">Direct Image Link</a>[/img], [center]Centered Text[/center],<br> [link]<a href="https://youtube.com/" target="_BLANK">https://youtube.com/</a>[/link], [color=<font color="green">GREEN</font>]<font color="green">Green Text!</font>[/color], [*] - Dotted [/*]</p><br>
			...
			?>
			<h1 style="color:black">Thread unavailable</h1>
	...
	if ($new_thread_category !== false && $new_thread_cid !== false) {
		...
		if ($category !== false) {
			...
			if ($access) {
				?>
				<h1 style="color:black">Create new thread</h1>
				<form type="" method="post" style="width:775px">
	...
	if ($getCat !== false) {
		...
		if ($category !== false) {
			...
			//foreach($charData)
			echo "<h1 style='color:black'><a href='forum.php'>Forum</a> Board: ". $category['name'] ."</h1>";
			...
			if ($threads !== false) {
				?>
				<table class="znoteTable table table-bordered table-striped table-hover" id="forumThreadTable" style="width:775px">
					<tr class="yellow">
			...
			if ($charCount > 0) {
				if ($category['closed'] == 0  || $admin) {
					...
					<form action="" method="post" style="width:775px">
						...
						<br></br><center><input type="submit" value="Create new thread" class="btn btn-primary"></center>
					</form>
					<?php
	...
	$guildboard = false;
	?>
	<table class="znoteTable table-striped table-hover" id="forumCategoryTable" style="width:775px">
		<tr class="yellow" >
			<th>Forum Boards</th>
			<?php
			$guild = false;
				...
				if ($access) {
					...
					if ($admin) {
						...
						<td style="margin: 0px; padding: 0px; width: 100px;">
							<form action="" method="post">
								<input type="hidden" name="admin_category_id" value="<?php echo $category['id']; ?>">
								<input type="submit" name="admin_category_edit" value="Edit"  class="btn btn-warning">
							</form>
						</td>
						<td style="margin: 0px; padding: 0px; width: 100px;">
							<form action="" method="post">
								<input type="hidden" name="admin_category_id" value="<?php echo $category['id']; ?>">
								<input type="submit" name="admin_category_delete" value="Delete"  class="btn btn-danger">
							</form>
						</td>
						<?php
					}
	...
	if ($guildboard !== false && $guild || $guildboard !== false && $admin) {
		//
		?>
		<table class=" table-striped table-hover znoteTable" id="forumCategoryTable" style="width:775px">
			<tr class="yellow">
				<th>Guild Boards</th>
				<?php
				foreach($charData as $char) {
					...
					if ($admin) {
						...
						<td style="margin: 0px; padding: 0px; width: 100px;">
							<form action="" method="post">
								...
								<input type="submit" name="admin_category_edit" value="Edit" style="" class="btn btn-warning">
							</form>
						</td>
						<td style="margin: 0px; padding: 0px; width: 100px;">
							<form action="" method="post">
	...
	if ($admin) {
		?>
		<h2 style="color:black">Create board:</h2>
		<form action="" method="post" style="width:775px">
